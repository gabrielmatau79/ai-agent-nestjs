 ...
 *
 * Stores chat messages per session using Redis lists with optional window size trimming.
 * Implements NestJS lifecycle hooks for graceful startup and shutdown.
 */
export class RedisMemory implements AgentMemory, OnModuleInit, OnModuleDestroy {
  ...
  constructor(windowSize: number, redisUrl: string) {
    ...
    this.logger.log(`🧠 RedisMemory initialized with window size ${windowSize}`)
  }
  ...
  async getHistory(sessionId: string): Promise<ChatMessage[]> {
    try {
      ...
      this.logger.debug(`📤 Fetched ${messages.length} messages from session ${sessionId}`)
      return messages
    } catch (error) {
  ...
  /**
   * Adds a new message to the Redis list for a session.
   * Automatically trims the list to maintain the window size and sets an expiration.
   *
